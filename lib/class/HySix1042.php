...
class HySix1042 extends HySix{
    ...
    protected function controller_exec1(){
        ...
        if(count($list_getvideopinglun) > 0){
            ...
            //搜索评论回复表
            $sql_where_back = " where vid='".$this->nowid."' and fplid in (" . $plstr .")";
            $sql_pl_back = "select id,userid,content,create_datetime, plid, userdata, fplid
            ...
            if(count($list_pl_back) > 0){//搜索结果为空
                ...
                foreach($list_getvideopinglun as $keygp => $valgp) {
                    ...
                    $list_getvideopinglun[$keygp]['fplid'] = $valgp['id'];
...
//                        var_dump(base64_decode($v_plb['content']));die;
//var_dump($v_plb);die;
//                        echo $valgp['id'];
//                        var_dump($list_pl_back[$k_plb]);die;
                        if($valgp['id'] == $v_plb['fplid']) {
//                            var_dump($list_pl_back[$k_plb]);die;
                            //存入评论内容
                            $list_pl_back[$k_plb]['content'] = base64_decode($v_plb['content']);
